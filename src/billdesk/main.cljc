(ns billdesk.main
  "Kotodama WASM entrypoint for the billdesk clean-room actor (L4) — Clojure port.
  ...
  facts (`billdesk.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "billdesk")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"billdesk.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "billdesk-compat" :tier tier :entities entities} 200])
...
(defn start! [] :billdesk-compat/ready)
